...
class D3D12RayTracingCubeLayer :public BlackPearl::Layer {

public:

	D3D12RayTracingCubeLayer(const std::string& name)
		: Layer(name)
	{

		m_BasicRenderer = DBG_NEW BlackPearl::BasicRenderer();
		m_D3D12RayTracingCubeRenderer = DBG_NEW BlackPearl::D3D12RayTracingCubeRenderer();
		...
		//m_PointLightObj = CreateLight(BlackPearl::LightType::PointLight);
		/***************************************** Scene ********************************************************/
	...
	virtual ~D3D12RayTracingCubeLayer() {
	...
	//	m_RayTracingRenderer->Render();
		...
		m_D3D12RayTracingCubeRenderer->Render(*(m_MainCamera->GetObj()->GetComponent<BlackPearl::PerspectiveCamera>()));
...
private:
	...
	BlackPearl::D3D12RayTracingCubeRenderer* m_D3D12RayTracingCubeRenderer;
