...
void main()
{
	...
	for(int i=-CORE_SIZE/2;i<=CORE_SIZE/2;i++){
		blurTexCoords[i+CORE_SIZE/2] = center + vec2(float(pixelSize*i),0.0);
...
//FragColor.a传到 vertical blur来判断是否需要guanssian blur
void main(){
			...
			if(u_Settings.showBlurArea)
				...
				for(int i=0;i<CORE_SIZE;i++){
					...
					if(texture(u_FinalScreenTexture, blurTexCoords[i]).a == 1&& i_objectId == ObjectId)
						color+=texture(u_FinalScreenTexture, blurTexCoords[i]).rgb*kernel31[i];
					else
						color+=texture(u_FinalScreenTexture, blurTexCoords[CORE_SIZE/2]).rgb*kernel31[i];
