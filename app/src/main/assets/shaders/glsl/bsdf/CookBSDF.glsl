...
vec3 evaluateCookBRDF(MaterialConstant mat, SurfaceGeometry geom, vec3 lightDir) {
...
vec4 ShadeSurface(in LightConstants light, in SurfaceGeometry geom, in MaterialConstant material){

   //Evaluate BRDF
    vec3 brdf = evaluateCookBRDF(material, geom, light.direction);
