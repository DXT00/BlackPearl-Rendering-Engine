...
/*reference code from https://yangwc.com/2019/06/11/Voxelization/ */
...
void main(){
	...
	for(uint i = 0; i < 3; ++i){
		...
		if(p.z > p.x && p.z > p.y){
			ClipPos[i] = vec4(worldPositionFrag.x, worldPositionFrag.y, 0, 1);
		}
		else if (p.x > p.y && p.x > p.z){
			ClipPos[i]  = vec4(worldPositionFrag.y, worldPositionFrag.z, 0, 1);
		} 
		else {
			ClipPos[i]  = vec4(worldPositionFrag.x, worldPositionFrag.z, 0, 1);
		}
