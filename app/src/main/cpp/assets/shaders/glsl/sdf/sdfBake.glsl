...
void main(){
    ...
    vec3 worldPos = g_Gdf.clipmapCenter + (vec3(voxelCoord) - g_Gdf.clipmapDimension *0.5) * g_Gdf.voxelSize; // 64x64x64 纹理中心对齐  [-32,32] * g_Gdf.voxelSize
...
//	obj.pos = vec3(objsInfo[objId*8+5],objsInfo[objId*8+6],objsInfo[objId*8+7]);
    ...
    float d = GLOBAL_SDF_WORLD_SIZE;
    float distance;
    for(int i = 0; i< g_Gdf.objsCnt; i++)
    {
      DFObjectConstants obj = g_objsInfo[i];
      float distance = boxSDF(worldPos, obj.center, obj.extend * 0.5);
      d = min(d, distance);

   

    }
     float ndcDistacne = d/(g_Gdf.clipmapDimension.x * g_Gdf.voxelSize);// [-1,1]
     imageStore(t_DistanceFieldTexture, voxelCoord, vec4((ndcDistacne+1.0)*0.5, 0.0, 0.0, 0.0)); //[0,1]
