    ...
    */
    ...
    void Texture::Init(
        ...
        if (data != nullptr || !m_Path.empty()) {
            ...
            if (!AssetManager::IsFileExist(m_Path)) {
                GE_CORE_WARN("file %s does not exist!", m_Path.c_str());
...
#undef LoadImage
#endif
            unsigned char *fileData = AssetManager::LoadImage(m_Path, width, height, nrChannels);


            //unsigned char* fileData = stbi_load_util(m_Path.c_str(), &width, &height, &nrChannels, 0);
            ...
            if(!textureData){
                GE_CORE_ERROR("fail to load texture data!");
                return;
            }
        }
