...
typedef int32_t(*PFN_ANativeWindow_setBuffersTransform)(struct ANativeWindow* window, int32_t transform);
...
//static TAutoConsoleVariable<int32_t> CVarAndroidGLESFlipYMethod(
	...
	,sampleSamples(0)
{
	...
	int DepthBufferPreference = (int)AndroidWindow::GetDepthBufferPreference();
	...
	if( CurrentContext != InContext)
	{
		...
		else
		{
			...
			if (!bSupportsKHRSurfacelessContext && InContext != EGL_NO_CONTEXT && InSurface == EGL_NO_SURFACE)
			{
				...
				if (PImplData->auxSurface == EGL_NO_SURFACE)
				{
					GE_ASSERT(PImplData->auxSurface != EGL_NO_SURFACE, ("eglCreatePbufferSurface error : 0x%x"), eglGetError());
				}
...
void AndroidEGL::CreateEGLRenderSurface(ANativeWindow* InWindow, bool bCreateWndSurface)
{
	...
	if (PImplData->eglSurface != EGL_NO_SURFACE)
	{
		//FPlatformMisc::LowLevelOutputDebugStringf(("AndroidEGL::CreateEGLRenderSurface() Already initialized: %p"), PImplData->eglSurface);
		return;
	}
...
//		}
		...
		//FPlatformMisc::LowLevelOutputDebugStringf( ("AndroidEGL::CreateEGLRenderSurface() %p" ), PImplData->eglSurface);

		if(PImplData->eglSurface == EGL_NO_SURFACE )
		{
			GE_ASSERT(PImplData->eglSurface != EGL_NO_SURFACE, ("eglCreateWindowSurface error : 0x%x"), eglGetError());
			ResetInternal();
		}
		...
		*/
		...
		GE_ASSERT(result == EGL_TRUE, ("eglQuerySurface error : 0x%x"), eglGetError());
	}
	else
	{
		...
		GE_ASSERT(PImplData->eglWidth != 0, ("eglWidth is ZERO; could be a problem!"));
		GE_ASSERT(PImplData->eglHeight != 0, ("eglHeight is ZERO; could be a problem!"));
		pbufferAttribs[1] = PImplData->eglWidth;
		pbufferAttribs[3] = PImplData->eglHeight;

		//FPlatformMisc::LowLevelOutputDebugStringf( ("AndroidEGL::CreateEGLRenderSurface(%d), eglSurface = eglCreatePbufferSurface(), %dx%d" ),
		...
		if(PImplData->eglSurface== EGL_NO_SURFACE )
		{
			GE_ASSERT(PImplData->eglSurface != EGL_NO_SURFACE, ("eglCreatePbufferSurface error : 0x%x"), eglGetError());
			ResetInternal();
		}
	}
}
...
void AndroidEGL::CreateEGLSharedSurface()
{
	...
	if (PImplData->auxSurface != EGL_NO_SURFACE)
	{
		//FPlatformMisc::LowLevelOutputDebugStringf(("AndroidEGL::CreateEGLSharedSurface() Already initialized: %p"), PImplData->auxSurface);
		return;
	}
	...
	//FPlatformMisc::LowLevelOutputDebugStringf( ("AndroidEGL::CreateEGLSharedSurface(), auxSurface = eglCreatePbufferSurface(), %dx%d" ),
...
//	bAlreadyInitialized = true;
	...
	GE_ASSERT(PImplData->eglDisplay == EGL_NO_DISPLAY)
	PImplData->eglDisplay = eglGetDisplay(EGL_DEFAULT_DISPLAY);
	GE_ASSERT(PImplData->eglDisplay, (" eglGetDisplay error : 0x%x "), eglGetError());
	...
	//FPlatformMisc::LowLevelOutputDebugStringf( ("EGL Extensions: \n%s" ), *Extensions );

	bSupportsKHRCreateContext = Extensions.find(("EGL_KHR_create_context")) != std::string::npos;
	bSupportsKHRSurfacelessContext = Extensions.find(("EGL_KHR_surfaceless_context"))!= std::string::npos;
	bSupportsKHRNoErrorContext = Extensions.find(("EGL_KHR_create_context_no_error"))!= std::string::npos;
	...
	{
		GE_ASSERT( 0, ("Attempt to initialize EGL with unexpected API type"));
	}

	GE_ASSERT( result == EGL_TRUE, ("eglBindAPI error: 0x%x "), eglGetError());
	...
	GE_ASSERT(result == EGL_TRUE , (" eglChooseConfig error: 0x%x"), eglGetError());

	GE_ASSERT(PImplData->eglNumConfigs != 0  ,(" eglChooseConfig num EGLConfigLists is 0 . error: 0x%x"), eglGetError());
	...
	for (uint32_t i = 0; i < PImplData->eglNumConfigs; i++)
	{
		...
		currScore = 0;
		currScore |= ((int64_t)math::min(math::abs(sb - PImplData->Parms.sampleBuffers), 15)) << 29;
		currScore |= ((int64_t)math::min(math::abs(sc - PImplData->Parms.sampleSamples), 31)) << 24;
		currScore |= math::min(
						math::abs(r - PImplData->Parms.redSize) +
						math::abs(g - PImplData->Parms.greenSize) +
						math::abs(b - PImplData->Parms.blueSize), 127) << 17;
		currScore |= math::min(math::abs(d - PImplData->Parms.depthSize), 63) << 11;
		currScore |= math::min(math::abs(1 - bNonLinearDepth), 1) << 10;
		currScore |= math::min(math::abs(s - PImplData->Parms.stencilSize), 31) << 6;
		currScore |= math::min(math::abs(a - PImplData->Parms.alphaSize), 31) << 0;
...
eglDisplay(EGL_NO_DISPLAY)
	...
	,CachedWindowRect(RHIRect(0,0,0,0))
{
}
...
void AndroidEGL::SetRenderContextWindowSurface()
{
    ...
    //FPlatformMisc::LowLevelOutputDebugStringf(("AndroidEGL::SetRenderContextWindowSurface  recreating context! tid: %d"),  FPlatformTLS::GetCurrentThreadId());
	UnBind();
	...
	PImplData->Window = (ANativeWindow*)AndroidWindow::GetHardwareWindow_EventThread();
	GE_ASSERT(PImplData->Window);
...
void AndroidEGL::ResizeRenderContextSurface()
{
	...
	if (PImplData->Window &&
		(PImplData->eglWidth != (PImplData->CachedWindowRect.width())
		|| PImplData->eglHeight != (PImplData->CachedWindowRect.height()))
		)
	{
//        UE_LOG(LogAndroid, Log,
//               TEXT("AndroidEGL::ResizeRenderContextSurface, PImplData->Window=%p, PImplData->eglWidth=%d, PImplData->eglHeight=%d!, CachedWidth=%d, CachedHeight=%d, tid: %d"),
//               PImplData->Window, PImplData->eglWidth, PImplData->eglHeight,
...
void AndroidEGL::ResizeSharedContextSurface()
{
	...
	{
//        UE_LOG(LogAndroid, Log,
//               TEXT("AndroidEGL::ResizeSharedContextSurface, PImplData->Window=%p, PImplData->eglWidth=%d, PImplData->eglHeight=%d!, CachedWidth=%d, CachedHeight=%d, tid: %d"),
//               PImplData->Window, PImplData->eglWidth, PImplData->eglHeight,
//               (PImplData->CachedWindowRect.Right - PImplData->CachedWindowRect.Left),
//               (PImplData->CachedWindowRect.Bottom - PImplData->CachedWindowRect.Top),
//               FPlatformTLS::GetCurrentThreadId());
...
extern void AndroidThunkCpp_SetDesiredViewSize(int32_t Width, int32_t Height);
...
{
	//F//PlatformMisc::LowLevelOutputDebugStringf(TEXT("AndroidEGL::InitRenderSurface %d, %d"), int(bUseSmallSurface), int(bCreateWndSurface));
	...
	if (!bUseSmallSurface)
	{
		RHIRect WindowSize = AndroidWindow::GetScreenRect();

		if (PImplData->CachedWindowRect.minX > 0 && PImplData->CachedWindowRect.minY > 0)
		{
...
void AndroidEGL::Init(APIVariant API, uint32_t MajorVersion, uint32_t MinorVersion, bool bDebug)
{
	...
	if (bSupportsKHRCreateContext)
	{
		...
		GE_ASSERT( Element < MaxElements, ("Too many elements in config list"));
	}
	else
	{
...
void AndroidEGL::GetDimensions(uint32_t& OutWidth, uint32_t& OutHeight)
{
...
uint32_t AndroidEGL::GetCurrentContextType()
{
	...
	{
		return CONTEXT_Shared;//make sure current context is valid one. //GE_ASSERT(GetCurrentContext != NULL);
	}
...
void AndroidEGL::UpdateBuffersTransform()
{
	...
	{
		int32_t BufferTransform = ANATIVEWINDOW_TRANSFORM_IDENTITY;
//
...
//		{
//			OnReinitWindowCallback(AndroidWindow::GetHardwareWindow_EventThread());
