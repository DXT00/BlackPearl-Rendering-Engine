...
namespace BlackPearl {
	...
	private:
		FOpenGLProgramBinaryCache(const std::string& InCachePath);
		~FOpenGLProgramBinaryCache();

		std::string GetProgramBinaryCacheFilePath() const;
	...
	private:
		...
		*/
		...
		bool OpenCacheWriteHandle(const std::string& ProgramCacheFilenameToWrite, bool bAppendToExisting);
		...
		void OnShaderPipelineCacheOpened(std::string const& Name, EShaderPlatform Platform, uint32 Count, const FGuid& VersionGuid, FShaderPipelineCache::FShaderCachePrecompileContext& ShaderCachePrecompileContext);
	...
	private:
		TRefCountPtr<FOpenGLProgramBinaryMapping> GetOrAddFileMapping(const std::string& ProgramCacheFilename, int32 ProgramCount, int64 Offset, int64 Size);

		std::string GetProgramBinaryCacheDir() const { return (CachePathRoot / CacheSubDir); }
		bool ReadProgramFile_Internal(uint32 ProgramsToRead, int64 EndOffset, const std::string& ProgramCacheFilename, FArchive& Ar);
		...
		std::map<FGuid, TRefCountPtr<FOpenGLProgramBinaryMapping>> MappedCacheFiles;
	};
}