...
namespace BlackPearl {
	struct FRHIGlobals
	{
		...
		int32_t PersistentThreadGroupCount = 0;

		/** The maximum number of mip-maps that a texture can contain. */
		int32_t MaxTextureMipCount = MAX_TEXTURE_MIP_COUNT;
		...
		struct FGpuInfo
		{
			...
			std::string AdapterInternalDriverVersion;
			std::string AdapterUserDriverVersion;
			std::string AdapterDriverDate;
		...
		int32_t MaxComputeDispatchDimension = ((1 << 16) - 1);
		...
		int32_t MaxTextureArrayLayers = 256;

		int32_t MaxTextureSamplers = 16;

		/** The maximum work group invocations allowed for compute shader. */
		int32_t MaxWorkGroupInvocations = 1024;
		...
		struct FRayTracing
		{
			...
			*/
			...
			uint32_t InstanceDescriptorSize = 0;
		...
		*/
		int32_t MinimumWaveSize = 0;
		int32_t MaximumWaveSize = 0;
		...
		***/
		...
		math::float3 MaxDispatchThreadGroupsPerDimension = math::float3(0.0);
		...
		*/
		...
		math::float2 DefaultMSAASampleOffsets[1 + 2 + 4 + 8 + 16];
	...
	extern  FRHIGlobals GRHIGlobals;
