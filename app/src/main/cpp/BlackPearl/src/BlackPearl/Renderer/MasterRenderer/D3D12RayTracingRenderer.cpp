...
namespace BlackPearl {
	...
	const wchar_t* D3D12RayTracingRenderer::c_raygenShaderName = L"MyRaygenShader";
	const wchar_t* D3D12RayTracingRenderer::c_closestHitShaderName = L"MyClosestHitShader";
	const wchar_t* D3D12RayTracingRenderer::c_missShaderName = L"MyMissShader";
	...
	void D3D12RayTracingRenderer::OnDeviceLost()
	{
	...
	void D3D12RayTracingRenderer::OnDeviceRestored()
	{
		CreateDeviceDependentResources();
		CreateWindowSizeDependentResources();
	}

	void D3D12RayTracingRenderer::Init()
	{
	...
	void D3D12RayTracingRenderer::Render()
	{
	...
	void D3D12RayTracingRenderer::OnSizeChanged(UINT width, UINT height, bool minimized)
	{
	}

	void D3D12RayTracingRenderer::OnDestroy()
	{
	...
	void D3D12RayTracingRenderer::DoRaytracing()
	{
		...
		auto DispatchRays = [&](auto* commandList, auto* stateObject, auto* dispatchDesc)
		{
			...
			dispatchDesc->Width = Application::Get().GetWindow().GetWidth();
			dispatchDesc->Height = Application::Get().GetWindow().GetHeight();
	...
	void D3D12RayTracingRenderer::CreateDeviceDependentResources()
	{
	...
	void D3D12RayTracingRenderer::CreateWindowSizeDependentResources()
	{
		CreateRaytracingOutputResource();
	...
	void D3D12RayTracingRenderer::ReleaseDeviceDependentResources()
	{
	...
	void D3D12RayTracingRenderer::ReleaseWindowSizeDependentResources()
	{
		m_raytracingOutput.Reset();
	}

	void D3D12RayTracingRenderer::CreateRaytracingInterfaces()
	{
	...
	void D3D12RayTracingRenderer::SerializeAndCreateRaytracingRootSignature(D3D12_ROOT_SIGNATURE_DESC& desc, ComPtr<ID3D12RootSignature>* rootSig)
	{
		auto device = m_deviceResources->GetD3DDevice();
		...
		ThrowIfFailed(device->CreateRootSignature(1, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&(*rootSig))), L"fail");
	}

	void D3D12RayTracingRenderer::CreateRootSignatures()
	{
   ...
   // This is a root signature that is shared across all raytracing shaders invoked during a DispatchRays() call.
		{
			...
			rootParameters[(UINT)GlobalRootSignatureParams::OutputViewSlot].InitAsDescriptorTable(1, &UAVDescriptor);
		...
		// This is a root signature that enables a shader to have unique arguments that come from shader tables.
		{
			...
			rootParameters[(UINT)LocalRootSignatureParams::ViewportConstantSlot].InitAsConstants(SizeOfInUint32(m_rayGenCB), 0, 0);
...
// This is a root signature that enables a shader to have unique arguments that come from shader tables.
	void D3D12RayTracingRenderer::CreateLocalRootSignatureSubobjects(CD3DX12_STATE_OBJECT_DESC* raytracingPipeline)
	{
	...
	void D3D12RayTracingRenderer::CreateRaytracingPipelineStateObject()
	{
	...
	void D3D12RayTracingRenderer::CreateDescriptorHeap()
	{
		auto device = m_deviceResources->GetD3DDevice();
	...
	void D3D12RayTracingRenderer::CreateRaytracingOutputResource()
	{
		auto device = m_deviceResources->GetD3DDevice();
		auto backbufferFormat = m_deviceResources->GetBackBufferFormat();

		// Create the output resource. The dimensions and format should match the swap-chain.
		auto uavDesc = CD3DX12_RESOURCE_DESC::Tex2D(backbufferFormat, Application::Get().GetWindow().GetWidth(), Application::Get().GetWindow().GetHeight(), 1, 1, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	...
	void D3D12RayTracingRenderer::BuildGeometry()
	{
		auto device = m_deviceResources->GetD3DDevice();
		Index indices[] =
		{
		...
		float offset = 0.5f;//0.7
		D3D12RayTracingRenderer::Vertex vertices[] =
		{
	...
	void D3D12RayTracingRenderer::BuildAccelerationStructures()
	{
		auto device = m_deviceResources->GetD3DDevice();
		auto commandList = m_deviceResources->GetCommandList();
	...
	void D3D12RayTracingRenderer::BuildShaderTables()
	{
		auto device = m_deviceResources->GetD3DDevice();
	...
	void D3D12RayTracingRenderer::CalculateFrameStats()
	{
	}

	UINT D3D12RayTracingRenderer::AllocateDescriptor(D3D12_CPU_DESCRIPTOR_HANDLE* cpuDescriptor, UINT descriptorIndexToUse)
	{
