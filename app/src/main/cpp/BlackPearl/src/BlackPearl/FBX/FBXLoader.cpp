...
namespace BlackPearl {
	...
	namespace
	{
		...
		inline glm::vec3 fixOrientation(const glm::vec3& v, Orientation orientation)
		{
			switch (orientation)
			{
		...
		inline Transform getTransform(const ofbx::Object* object, Orientation orientation)
		{
			...
			glm::vec3 localPos = (fixOrientation({ static_cast<float>(p.x), static_cast<float>(p.y), static_cast<float>(p.z) }, orientation));
			ofbx::Vec3 r = object->getLocalRotation();
			glm::vec3 localOrientation = (fixOrientation(glm::vec3(static_cast<float>(r.x), static_cast<float>(r.y), static_cast<float>(r.z)), orientation));
		...
		inline void loadMesh(Model* model, const std::string& fileName, const ofbx::IScene* scene, std::vector<const ofbx::Object*>& sceneBone, std::vector<std::shared_ptr<IResource>>& outRes, Orientation orientation, MaterialShader* shader)
		{
			...
			if (scene->getMeshCount() > 0)
			{
				...
				for (int32_t i = 0; i < scene->getMeshCount(); ++i)
				{
					...
					for (int32_t i = 0; i < vertexCount; ++i)
					{
					    ...
					    fixOrientation(glm::vec3(tmpPos.x, tmpPos.y, tmpPos.z), orientation);
					    
						updateAABB(model, vertex.position, isFirstVertex);
						if (isFirstVertex)
							isFirstVertex = false;
					    ...
					    {																																						
							...
							fixOrientation(glm::vec3(vertex.normal.x, vertex.normal.y, vertex.normal.z), orientation);
					...
					/*	if (colors)																																				
						...
						if (tangents) {
							...
							fixOrientation(glm::vec3(tmpTangent.x, tmpTangent.y, tmpTangent.z), orientation);
					...
					for (auto i = 0; i < fbxMesh->getMaterialCount(); i++)
					{
						...
						pbrMaterialsCache.emplace_back(loadMaterial(material, shader, false));
					}
	...
	Model* FBXLoader::load(const std::string& fileName, const ModelDesc& desc, const std::string& extension, std::vector<std::shared_ptr<IResource>>& outRes) const
	{
		...
		std::vector<uint8_t> fbxData = AssetManager::LoadBuffer(fileName);
//		mio::mmap_source mmap(fileName);
		...
		auto           scene = ofbx::load((uint8_t*)fbxData.data(), fbxData.size(), flags);
		...
		loadMesh(model, fileName, scene, sceneBone, outRes, orientation, m_FBXDefaultShader);
