...
namespace CrossCompiler
{
	...
	enum class EPackedTypeName : int8_t
	{
		HighP = 'h',
		MediumP = 'm',
		LowP = 'l',
	...
	enum class EPackedTypeIndex : int8_t
	{
		HighP = 0,
		MediumP = 1,
		LowP = 2,
	...
	enum
	{
		PACKED_TYPENAME_HIGHP = (int32_t)EPackedTypeName::HighP,	// Make sure these enums match hlslcc
		PACKED_TYPENAME_MEDIUMP = (int32_t)EPackedTypeName::MediumP,
		PACKED_TYPENAME_LOWP = (int32_t)EPackedTypeName::LowP,
		PACKED_TYPENAME_INT = (int32_t)EPackedTypeName::Int,
		PACKED_TYPENAME_UINT = (int32_t)EPackedTypeName::Uint,
		PACKED_TYPENAME_SAMPLER = (int32_t)EPackedTypeName::Sampler,
		PACKED_TYPENAME_IMAGE = (int32_t)EPackedTypeName::Image,

		PACKED_TYPEINDEX_HIGHP = (int32_t)EPackedTypeIndex::HighP,
		PACKED_TYPEINDEX_MEDIUMP = (int32_t)EPackedTypeIndex::MediumP,
		PACKED_TYPEINDEX_LOWP = (int32_t)EPackedTypeIndex::LowP,
		PACKED_TYPEINDEX_INT = (int32_t)EPackedTypeIndex::Int,
		PACKED_TYPEINDEX_UINT = (int32_t)EPackedTypeIndex::Uint,
		PACKED_TYPEINDEX_SAMPLER = (int32_t)EPackedTypeIndex::Sampler,
		PACKED_TYPEINDEX_IMAGE = (int32_t)EPackedTypeIndex::Image,
		PACKED_TYPEINDEX_BUFFER = (int32_t)EPackedTypeIndex::ConstantBuffer,

		PACKED_TYPEINDEX_MAX = (int32_t)EPackedTypeIndex::Max,
	};

	static FORCEINLINE uint8_t ShaderStageIndexToTypeName(uint8_t ShaderStage)
	{
		switch (ShaderStage)
		{
	...
	static FORCEINLINE uint8_t PackedTypeIndexToTypeName(uint8_t ArrayType)
	{
		switch (ArrayType)
		{
		...
		//checkf(0, TEXT("invalid value: PackedTypeIndexToTypeName(ArrayType = %d)"), (int32_t)ArrayType);
		return 0;
	}

	static FORCEINLINE uint8_t PackedTypeNameToTypeIndex(uint8_t ArrayName)
	{
		switch (ArrayName)
		{
		...
		//checkf(0, TEXT("invalid value: PackedTypeNameToTypeIndex(ArrayName = %d)"), (int32_t)ArrayName);
		return 0;
	}
...
#pragma warning(error : 4596)
	struct FShaderBindingInOutMask
	{
		...
		static constexpr int32_t MaxIndex = (sizeof(decltype(FShaderBindingInOutMask::Bitmask)) * 8u) - 1u;

		/** Index to mark the binding of a depth-stencil output resource. */
		static constexpr int32_t DepthStencilMaskIndex = MaxIndex;

		/** Sets the specified bitfield in this bitmask and validates its index boundary. */
		FORCEINLINE void EnableField(int32_t Index)
		{
		...
		FORCEINLINE bool IsFieldEnabled(int32_t Index) const
		{
			//ensure(Index >= 0 && Index <= FShaderBindingInOutMask::MaxIndex);
...
#pragma warning(pop)
	...
	//	uint8_t					NumUAVs;
	//	uint8_t					NumAccelerationStructures;
