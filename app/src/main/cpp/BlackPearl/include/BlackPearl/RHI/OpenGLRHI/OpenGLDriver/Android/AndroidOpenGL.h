...
typedef GLboolean(GL_APIENTRYP PFNeglPresentationTimeANDROID)(EGLDisplay dpy, EGLSurface surface,
                                                                  EGLnsecsANDROID time);

typedef GLboolean(GL_APIENTRYP PFNeglGetNextFrameIdANDROID)(EGLDisplay dpy, EGLSurface surface,
                                                                EGLuint64KHR *frameId);

typedef GLboolean(GL_APIENTRYP PFNeglGetCompositorTimingANDROID)(EGLDisplay dpy, EGLSurface surface,
                                                                     EGLint numTimestamps, const EGLint *names,
                                                                     EGLnsecsANDROID *values);

typedef GLboolean(GL_APIENTRYP PFNeglGetFrameTimestampsANDROID)(EGLDisplay dpy, EGLSurface surface,
                                                                    EGLuint64KHR frameId, EGLint numTimestamps,
                                                                    const EGLint *timestamps, EGLnsecsANDROID *values);
typedef GLboolean(GL_APIENTRYP PFNeglQueryTimestampSupportedANDROID)(EGLDisplay dpy, EGLSurface surface,
                                                                         EGLint timestamp);
...
namespace BlackPearl {
    ...
    struct FAndroidOpenGL : public FOpenGLES {
        ...
        static void GetQueryObject(GLuint QueryId, FOpenGLBase::EQueryMode QueryMode, GLuint *OutResult);

        static void GetQueryObject(GLuint QueryId, FOpenGLBase::EQueryMode QueryMode, GLuint64 *OutResult);
        ...
        static FORCEINLINE void DeleteSync(GLsync Sync) {
            if (Configuration::GUseThreadedRendering) {
                glDeleteSync(Sync);
            }
        }

        static FORCEINLINE GLsync FenceSync(GLenum Condition, GLbitfield Flags) {
            return Configuration::GUseThreadedRendering ? glFenceSync(Condition, Flags) : 0;
        }
            ...
            if (Configuration::GUseThreadedRendering) {
                ...
                switch (Result) {
                    case GL_ALREADY_SIGNALED:
                        return FR_AlreadySignaled;
                    case GL_TIMEOUT_EXPIRED:
                        return FR_TimeoutExpired;
        ...
        enum class EImageExternalType : uint8_t {
            None,
            ImageExternal100,
            ImageExternal300,
        ...
        static FORCEINLINE GLint GetMaxComputeUniformComponents() {
            GE_ASSERT(MaxComputeUniformComponents != -1);
        ...
        static FORCEINLINE GLint GetMaxComputeUAVUnits() {
            GE_ASSERT(MaxComputeUAVUnits != -1);
            return MaxComputeUAVUnits;
        }
        ...
        static FORCEINLINE GLint GetMaxPixelUAVUnits() {
            GE_ASSERT(MaxPixelUAVUnits != -1);
            return MaxPixelUAVUnits;
        }
        ...
        static int32_t GLMajorVerion;
        static int32_t GLMinorVersion;
        ...
        static std::vector<uint8_t> DispatchAndWaitForRemoteGLProgramCompile(const std::vector<uint8_t> ContextData,
