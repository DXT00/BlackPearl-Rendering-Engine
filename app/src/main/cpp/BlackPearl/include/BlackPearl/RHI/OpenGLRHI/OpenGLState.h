...
struct FOpenGLStream
{
	...
	uint32_t Offset;
	uint32_t Divisor;
	uint32_t NumVertices;//�ж��ٸ����㣬ÿ�������ж�������(pos, normal, uv�ȣ�
...
struct FOpenGLContextState final : public FOpenGLCommonState
{
	...
	virtual void InitializeResources(int32_t NumCombinedTextures, int32_t NumCombinedUAVUnits) override
	{
		...
		GE_ASSERT(NumCombinedUAVUnits <= sizeof(ActiveUAVMask) * 8, ("Not enough bits in ActiveUAVMask to store {:d} UAV units"), NumCombinedUAVUnits);
	}
