 ...
 */
	...
	public:
		...
		inline FOctreeChildNodeRef GetContainingChild(const AABB& QueryBounds) const
		{
	...
	public:
		...
		*/
		...
		inline void FindNearbyElements(const math::float3& Position, const IterateBoundsFunc& Func) const
		{
			...
			FindNearbyElementsInternal(0, RootNodeContext, AABB(Position, math::float3(0.0),false), Func);
		}
		...
		double GetNodeLevelExtent(int32_t Level) const
		{
			...
			return RootNodeContext.Bounds.m_Extent.x * std::pow((1.0f + 1.0f / (double)FOctreeNodeContext::LoosenessDenominator) / 2.0f, double(ClampedLevel));
		}
