...
//	else
    ...
    gl_Position =   g_Transform.matModel * vec4(aPos,1.0);//vec4(worldPositionGeom, 1.0); g_View.matProjectionView *
...
void main(){
	...
	for(uint i = 0; i < 3; ++i){
		...
		vec3 normal_worldPositionFrag = (worldPositionGeom[i]- g_Voxel.center) / g_Voxel.areaExtent; //worldPositionGeom[i];////GetVoxelLocalPos(worldPositionGeom[i]) / g_Voxel.areaExtent;//
...
void main(){
    ...
    MaterialSample mat;
    MaterialTextureSample textures = SampleMaterialTexturesAuto(texCoordFrag.xy);

#if USE_ALBEDO_MAP
    mat.albedo = textures.albedo.xyz;
#else
    mat.albedo = g_Mat.albedo.xyz;
#endif
	...
	vec3 voxel = scaleAndBias(normalWorldPositionFrag);//normalWorldPositionFrag;
	ivec3 dim = ivec3(g_Voxel.dimension);//imageSize(texture3D);// retrieve the dimensions of an image
	vec4 res = vec4(vec3(color), 1);
    //vec4 res = vec4(1.0,1.0, 0.0, 1);
