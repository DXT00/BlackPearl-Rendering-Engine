...
namespace BlackPearl {
    const wchar_t* D3D12MeshShaderRenderer::c_meshFilename = L"..\\Assets\\Dragon_LOD0.bin";

    const wchar_t* D3D12MeshShaderRenderer::c_meshShaderFilename = L"MeshletMS.cso";
    const wchar_t* D3D12MeshShaderRenderer::c_pixelShaderFilename = L"MeshletPS.cso";
    ...
    void D3D12MeshShaderRenderer::MoveToNextFrame()
    {
        ...
        //在command queue的末尾增加一个将fence value设为curFenceValue的命令
        ...
        {
            //如果小于backbuffer frame的fence，说明本帧的被期望命令还没有全部完成
            //那么我们可以增加一个达到后的触发事件，在fence value到达
            // m_FenceValues[m_FrameIndex]后告诉我们并停止CPU端的阻塞
            ThrowIfFailed(m_Fence->SetEventOnCompletion(m_FenceValues[m_FrameIndex], m_FenceEvent));
            //等待一直到fence value达到 mCurrentFence
    ...
    {
        for (auto& m : model->GetComponent<MeshRenderer>()->GetMeshes())
        {
    ...
    void D3D12MeshShaderRenderer::UpdateCameraMatrices(const Camera& camera)
    {
        ...
        XMMATRIX view = XMMatrixLookAtLH(cameraPosition, cameraPosition + front, up);
	...
	void D3D12MeshShaderRenderer::PopulateCommandList()
	{
        ...
        for (auto& obj : m_Scene->GetObjects()) {
            for (auto& mesh : obj->GetComponent<MeshRenderer>()->GetMeshes()) {
